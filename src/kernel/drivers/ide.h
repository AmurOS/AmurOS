...
void ide_read_buffer(unsigned char channel, unsigned char reg, unsigned int *buffer,
   ...
   if (reg > 0x07 && reg < 0x0C)
      ide_write(channel, ATA_REG_CONTROL, 0x80 | channels[channel].nIEN);
   asm __volatile__("pushw %es; movw %ds, %ax; movw %ax, %es");
   if (reg < 0x08)
...
unsigned char ide_print_error(unsigned int drive, unsigned char err) {
   ...
   __sh_error("IDE:");
   if (err == 1) {__sh_error("- Device Fault\n     "); err = 19;}
   else if (err == 2) {
      unsigned char st = ide_read(ide_devices[drive].Channel, ATA_REG_ERROR);
      if (st & ATA_ER_AMNF)   {__sh_error("- No Address Mark Found\n     ");   err = 7;}
      if (st & ATA_ER_TK0NF)   {__sh_error("- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_ABRT)   {__sh_error("- Command Aborted\n     ");      err = 20;}
      if (st & ATA_ER_MCR)   {__sh_error("- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_IDNF)   {__sh_error("- ID mark not Found\n     ");      err = 21;}
      if (st & ATA_ER_MC)   {__sh_error("- No Media or Media Error\n     ");   err = 3;}
      if (st & ATA_ER_UNC)   {__sh_error("- Uncorrectable Data Error\n     ");   err = 22;}
      if (st & ATA_ER_BBK)   {__sh_error("- Bad Sectors\n     ");       err = 13;}
   } else  if (err == 3)           {__sh_error("- Reads Nothing\n     "); err = 23;}
     else  if (err == 4)  {__sh_error("- Write Protected\n     "); err = 8;}
   __sh_error("- [%s %s] %s\n",
...
unsigned int BAR4) {
   ...
   for (int i = 0; i < 2; i++)
      for (int j = 0; j < 2; j++) {
         ...
         __std__sleep(1); // Wait 1ms for drive select to work.
 
         // (II) Send ATA Identify Command:
         ide_write(i, ATA_REG_COMMAND, ATA_CMD_IDENTIFY);
         __std__sleep(1); // This function should be implemented in your OS. which waits for 1 ms.
   ...
   for (int i = 0; i < 4; i++)
      if (ide_devices[i].Reserved == 1) {
         __sh_message(" Found %s Drive %dGB - %s\n",
   ...
   /* Режимы чтения/записи ATA/ATAPI:
 * ++++++++++++++++++++++++++++++++
 * Режимы адресации:
 * ================
 * - Режим LBA28. (+)
 ...
 * Режимы чтения:
 * ================
 * - Режимы PIO (0 : 6) (+) // Медленнее, чем DMA, но это не проблема.
 * - Режимы DMA с одним словом (0, 1, 2).
 * - Режимы DMA с двойным словом (0, 1, 2).
 * - Режимы Ultra DMA (0: 6).
 * Режимы опроса:
 * ================
 * - IRQs
 * - Статус опроса (+) // Подходит для выполнения одной задачи 
 */
