byte32i heap_start=0x2005000;
byte32i heap_end=0xa0000000;
...
void *kalloc(long unsigned int size) {
	...
	app->used=1;
	app->size=size;
	app->ptr=(byte32i *) search->ptr+search->size+1;
	list_append(app);	
...
/*void kinit() {
	...
	list_head->size=0;
	list_head->used=1;
	list_head->ptr=(byte32i *)heap_start;
	size=1;
